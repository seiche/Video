...
exports = module.exports = function Processor(command) {
    ...
    this._prepare(function(err) {
      ...
      ffmpegProc.stderr.on('data', function (data) {
        ...
        if (options.onProgress) {
					...
					self._makeProgress(data, self.metaData.durationsec);
				}
      });
    });
  };
  ...
  command.prototype.takeScreenshots = function(config, folder, callback) {
	...
	command.prototype._makeProgress = function(data, totalTime){
		var start = data.indexOf('time=');
		if(start > -1){
			start += 5;
			var end = data.indexOf('bitrate');
			var length = end - start;
			var substr = data.substr(start, length);
			var current = parseFloat(substr);
			var percent = current/totalTime * 100000;
